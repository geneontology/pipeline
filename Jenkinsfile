pipeline {
	...
	stage('Produce GAFs') {
		...
		dir('./go-site') {
			...
			withEnv(['MINERVA_CLI_MEMORY=32G', 'OWLTOOLS_MEMORY=128G', 'PATH+EXTRA=../bin', 'PYTHONHOME=', "VIRTUAL_ENV=${PWD}/target/env", "VIRTUAL_ENV2=${WORKSPACE}/go-site/pipeline/target/env"]){
			    sh 'env > env.txt'
			    sh 'cat env.txt'
			    sh 'echo $VIRTUAL_ENV'
