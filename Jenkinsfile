pipeline {
	...
	stage('Noctua Migrations') {
		...
		script {
			...
			checkout changelog: false, poll: false, scm: [$class: 'GitSCM', branches: [[name: TARGET_NOCTUA_MODELS_BRANCH]], extensions: [[$class: 'CloneOption', depth: 1, noTags: true, reference: '', shallow: true, timeout: 120], [$class: 'LocalBranch', localBranch: "**"]], userRemoteConfigs: [[url: 'https://github.com/geneontology/noctua-models.git', refspec: "+refs/heads/${env.TARGET_NOCTUA_MODELS_BRANCH}:refs/remotes/origin/${env.TARGET_NOCTUA_MODELS_BRANCH}"]]]
			...
			withEnv(['MINERVA_CLI_MEMORY=8G']){
			    ...
			    sh './bin/minerva-cli.sh --replace-obsolete -j blazegraph.jnl --ontology https://skyhook.berkeleybop.org/go-lego-reacto.owl'
			    // Dump them back out from the journal.
			    sh './bin/minerva-cli.sh --dump-owl-models -j blazegraph.jnl -f models/'
