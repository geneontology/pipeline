pipeline {
	...
	stage('GO-CAM JSON model generation') {
		...
		script {
			...
			withCredentials([file(credentialsId: 'skyhook-private-key', variable: 'SKYHOOK_IDENTITY')]) {
			    sh 'scp -o StrictHostKeyChecking=no -o IdentitiesOnly=true -o IdentityFile=$SKYHOOK_IDENTITY noctua-models-json.tgz skyhook@skyhook.berkeleybop.org:/home/skyhook/snapshot/products/json/'
			}
		    }
		}
	    }
		...
		// Run.
                ...
                sh "cd /opt/gocam-py && poetry run gocam translate-collection --max-workers $GOCAM_MAX_WORKERS --output $GOCAM_OUTPUT_DIR --batch-size $GOCAM_BATCH_SIZE --url http://skyhook.berkeleybop.org/snapshot/products/json/noctua-models-json.tgz"
