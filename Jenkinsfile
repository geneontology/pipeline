pipeline {
	...
	TARGET_BUCKET = 'go-data-testing-sandbox'
	...
	// master testing).
	ZENODO_REFERENCE_CONCEPT = 'nope'
	ZENODO_ARCHIVE_CONCEPT = 'nope'
	...
	AWS_CLOUDFRONT_DISTRIBUTION_ID = 'nope'
	AWS_CLOUDFRONT_RELEASE_DISTRIBUTION_ID = 'nope'

	///
	/// Ontobio Validation
	///
	VALIDATION_ONTOLOGY_URL="http://skyhook.berkeleybop.org/issue-203-archive-template-update/ontology/go.json"
	...
	MINERVA_INPUT_ONTOLOGIES = [
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/ontology/extensions/go-lego.owl"
	].join(" ")
	...
	GOLR_INPUT_ONTOLOGIES = [
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/ontology/extensions/go-gaf.owl",
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/ontology/extensions/gorel.owl",
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/ontology/extensions/go-modules-annotations.owl",
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/ontology/extensions/go-taxon-subsets.owl",
	...
	GOLR_INPUT_GAFS = [
	    //"http://skyhook.berkeleybop.org/issue-203-archive-template-update/products/annotations/paint_other.gaf.gz",
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/annotations/aspgd.gaf.gz",
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/annotations/goa_chicken.gaf.gz",
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/annotations/goa_chicken_complex.gaf.gz",
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/annotations/goa_uniprot_all_noiea.gaf.gz",
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/annotations/mgi.gaf.gz",
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/annotations/pombase.gaf.gz",
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/annotations/wb.gaf.gz"
	].join(" ")
	GOLR_INPUT_PANTHER_TREES = [
	    "http://skyhook.berkeleybop.org/issue-203-archive-template-update/products/panther/arbre.tgz"
	].join(" ")
	...
	// 	    sh 'find /opt/go-site/pipeline/target/groups -type f -regex "^.*.\\(gaf\\|gpad\\|gpi\\).gz$" -not -regex "^.*\\(\\-src.gaf\\|\\_noiea.gaf\\|\\_valid.gaf\\|paint_.*\\).gz$" -exec scp -o StrictHostKeyChecking=no -o IdentitiesOnly=true -o IdentityFile=$SKYHOOK_IDENTITY {} skyhook@skyhook.berkeleybop.org:/home/skyhook/$BRANCH_NAME/annotations \\;'
	...
	stage('Publish') {
		...
		withCredentials([file(credentialsId: 'aws_go_push_json', variable: 'S3_PUSH_JSON'), file(credentialsId: 's3cmd_go_push_configuration', variable: 'S3CMD_JSON'), string(credentialsId: 'aws_go_access_key', variable: 'AWS_ACCESS_KEY_ID'), string(credentialsId: 'aws_go_secret_key', variable: 'AWS_SECRET_ACCESS_KEY')]) {
			...
			withEnv(["PATH+EXTRA=${WORKSPACE}/go-site/bin:${WORKSPACE}/go-site/mypyenv/bin", 'PYTHONHOME=', "VIRTUAL_ENV=${WORKSPACE}/go-site/mypyenv", 'PY_ENV=mypyenv', 'PY_BIN=mypyenv/bin']){
				...
				// }else if( env.BRANCH_NAME == 'master' ){
				//     // Pass.
				// }else if( env.BRANCH_NAME == 'issue-203-archive-template-update' ){
				//     sh 'python3 ./scripts/s3-uploader.py -v --credentials $S3_PUSH_JSON --directory $WORKSPACE/mnt/$BRANCH_NAME/ --bucket go-data-product-daily/$START_DAY --number $BUILD_ID --pipeline $BRANCH_NAME'
				// }
