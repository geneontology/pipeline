pipeline {
	...
	TARGET_GO_SITE_BRANCH = 'issue-pipeline-204-suppress-iea-filter'
	...
	stage('Minerva generations') {
		...
		parallel(
			...
			script {
				...
				sh 'wget -N https://raw.githubusercontent.com/geneontology/go-site/master/scripts/collate-gpads.pl'
	...
	stage('Produce GAFs, TTLs, and journal (*)') {
		...
		withEnv(['JAVA_OPTS=-Xmx128G', 'OWLTOOLS_MEMORY=128G', 'BGMEM=128G', "ONTOLOGY=${VALIDATION_ONTOLOGY_URL}", "BRANCH_NAME=${BRANCH_NAME}"]){
		    ...
		    // sh 'export PATH=$WORKSPACE/pipeline/bin:$PATH'
		    sh 'env > env.txt'
		    sh 'cat env.txt'
